  ...
  extensions = {
    ...
    fslogix_config = {
      ...
      settings = jsonencode({
        ...
        commandToExecute = "powershell -ExecutionPolicy Unrestricted -File simplified-fslogix-config.ps1 -StorageAccountName '${module.fslogix_storage.name}' -ProfileShare 'profiles' -OfficeShare 'office-containers'"
      })
    },
